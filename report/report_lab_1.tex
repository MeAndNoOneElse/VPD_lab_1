...
Для описания динамики двигателя постоянного тока с постоянными магнитами рассмотрим два основных физических закона.

\paragraph{Второй закон Ньютона для вращательного движения:}
Суммарный вращающий момент, действующий на ротор, равен произведению момента инерции на угловое ускорение:
\begin{equation}
J \frac{d\omega(t)}{dt} = M_{\text{em}}(t) - M_{\text{нагр}}(t),
\label{eq:newton}
\end{equation}
где $J$ — момент инерции ротора (кг·м$^2$), $\omega$ — угловая скорость (рад/с), $M_{\text{em}}$ — электромагнитный момент двигателя (Н·м).
В данной работе двигатель не нагружен, поэтому $M_{\text{нагр}} = 0$.

Электромагнитный момент пропорционален току якоря:
\begin{equation}
M_{\text{em}}(t) = k_m I(t),
\label{eq:torque}
\end{equation}
где $k_m$ — коэффициент момента (Н·м/А), $I$ — ток в обмотках (А).

\paragraph{Закон Ома для цепи якоря:}
При вращении ротора в обмотках наводится противо-ЭДС $\varepsilon_i = -k_e \omega(t)$, препятствующая изменению тока. С учётом этого закон Ома принимает вид:
\begin{equation}
U(t) - k_e \omega(t) = I(t) R,
\label{eq:ohm}
\end{equation}
где $U$ — поданное на двигатель напряжение (В), $R$ — сопротивление обмоток (Ом), $k_e$ — коэффициент противо-ЭДС (В·с/рад).

\paragraph{Вывод дифференциального уравнения:}
Выразим ток из \eqref{eq:ohm}:
\begin{equation}
I(t) = \frac{U(t) - k_e \omega(t)}{R}.
\end{equation}
Подставим в \eqref{eq:torque}, а затем в \eqref{eq:newton}:
\begin{equation}
J \frac{d\omega(t)}{dt} = k_m \cdot \frac{U(t) - k_e \omega(t)}{R}.
\end{equation}
Раскроем скобки и перенесём слагаемое с $\omega$ в левую часть:
\begin{equation}
J \frac{d\omega(t)}{dt} + \frac{k_m k_e}{R} \omega(t) = \frac{k_m}{R} U(t).
\end{equation}
Разделим обе части на $J$:
\begin{equation}
\frac{d\omega(t)}{dt} + \frac{k_m k_e}{JR} \omega(t) = \frac{k_m}{JR} U(t).
\label{eq:diff_raw}
\end{equation}

\paragraph{Введение постоянной времени и коэффициента передачи:}
Введём электромеханическую постоянную времени $T_m$ и статический коэффициент передачи по напряжению $k_U$:
\begin{equation}
T_m = \frac{JR}{k_m k_e}, \qquad k_U = \frac{1}{k_e}.
\end{equation}
Тогда уравнение \eqref{eq:diff_raw} принимает классический вид апериодического звена первого порядка:
\begin{equation}
T_m \frac{d\omega(t)}{dt} + \omega(t) = k_U \cdot U(t).
\label{eq:aperiodic}
\end{equation}

\paragraph{Переход к управляющему сигналу в процентах:}
В экспериментальной установке напряжение подаётся в процентах от максимального ($U_{\text{max}} = 9$ В). Обозначим $u(t)$ — управляющий сигнал (\%). Тогда:
\begin{equation}
U(t) = \frac{u(t)}{100} U_{\text{max}}.
\end{equation}
Подставляя в \eqref{eq:aperiodic}, получаем:
\begin{equation}
T_m \frac{d\omega(t)}{dt} + \omega(t) = k \cdot u(t),
\label{eq:final}
\end{equation}
где
\begin{equation}
k = \frac{U_{\text{max}}}{100 \cdot k_e} \quad \text{(рад/(с·\%))}.
\end{equation}

\paragraph{Решение для ступенчатого входного сигнала:}
При подаче постоянного напряжения $u(t) = U_0 \cdot \mathbf{1}(t)$ решение уравнения \eqref{eq:final} имеет вид:
\begin{equation}
\omega(t) = k U_0 \left(1 - e^{-t/T_m}\right),
\label{eq:omega_solution}
\end{equation}
а угол поворота получается интегрированием скорости:
\begin{equation}
\theta(t) = \int_0^t \omega(\tau) d\tau = k U_0 \left( t - T_m \left(1 - e^{-t/T_m}\right) \right).
\label{eq:theta_solution}
\end{equation}

Таким образом, динамика двигателя EV3 в первом приближении описывается передаточной функцией апериодического звена:
\begin{equation}
W(s) = \frac{k}{T_m s + 1}.
\end{equation}
